...
import CommentSection from "@/app/(pages)/main/MainPage/posts/Comment/CommentModal";
...
export default function Post({ post }) {
  ...
  const [commentsCount, setCommentsCount] = useState(post.comments_count || 0); // Thêm state
  const [showComments, setShowComments] = useState(false); // Thêm state cho modal
  ...
  const handleSend = async (e) => {
    ...
    try {
      ...
      client.setToken(accessToken.value);
      const { data } = await client.post(`/posts/${post.id}/comments`, {
        content: text.trim(),
      });
      console.log("Comment response:", data);
      if (data.message === "Tạo bình luận thành công") {
        setText("");
  ...
  // Callback khi có comment mới từ modal
  const handleCommentAdded = () => {
    setCommentsCount((prev) => prev + 1);
  };

  // Hàm mở modal comments
      ...
      <Container sx={{ width: "480px" }} className="mt-6 relative">
        <Box className="flex justify-between items-center ">
          <Box className="flex items-center">
            ...
            />
            <span className="after:content-['•'] font-semibold ml-3 cursor-pointer">
              {post.user.username}
            </span>
        ...
        <Box className="mt-2 flex justify-between">
          ...
          <Box className="flex items-center gap-4">
            ...
            <Tooltip
              ...
              offset={0}
            >
              <Box className="cursor-pointer" onClick={handleShowComments}>
                <svg
        ...
        <Box className="text-sm">
          ...
          <Box className="mt-2">
            ...
            >
              Xem tất cả <span>{commentsCount}</span> bình luận
            </p>
          </Box>
